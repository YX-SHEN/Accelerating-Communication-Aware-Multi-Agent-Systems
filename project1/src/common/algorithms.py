...
# -- Unified numeric types and constants (switched via config) --
Real    = getattr(config, "DTYPE", np.float32)   # Single/double precision switch
DT      = getattr(config, "DT", 0.01)            # Time step Δt
EPS_DIV = getattr(config, "EPS_DIV", 1e-6)       # Denominator protection ε
    ...
    for iter in range(max_iter):
        ...
        for i in range(swarm_size):
            ...
            for j in [x for x in range(swarm_size) if x != i]:
                ...
                # Always record distance & aij (for rn/Jn statistics)
                distances_matrix[i, j] = rij
                distances_matrix[j, i] = rij
                neighbor_agent_matrix[i, j] = aij
                neighbor_agent_matrix[j, i] = aij

                # Only edges that "meet the threshold" have a value in the visualization matrix, otherwise set to 0
                if aij >= PT:
                    phi_rij = gij * aij
        ...
        # Core animation logic: call the plotting function after each iteration
        ...
        # --- New: Unified-style iteration log (does not affect calculation) ---
        if iter % LOG_EVERY == 0:
            print(f"[it={iter}] Jn={Jn[-1]:.4f} rn={rn[-1]:.4f}")

        # --- Original stopping logic + fixed-iters compatibility ---
        if len(Jn) > 19 and len(set(Jn[-20:])) == 1:
            if not Jn_converged:
                ...
                Jn_converged = True  # Prevent repeated printing
