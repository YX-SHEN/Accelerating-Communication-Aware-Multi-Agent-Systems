...
# Baseline: still call original algorithms (no logic changes)
from src.common import algorithms, plotting, utils

def main():
    """
    Run baseline (O(N^2)) algorithm: only change output/recording format, not algorithm logic.
    """
    # --- 0. Banner (unified style) ---
    print("\n--- Loading [V1: Baseline] Algorithm Engine ---\n")

    # --- 1. Result directory (unified naming style) ---
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    run_name = f"{timestamp}_baseline_grid{config.GRID_SIZE}_noise{getattr(config, 'noise_magnitude', 'N/A')}"
    ...
    # --- 4. Save results (unified filenames/prompts) ---
    ...
    with open(config_filepath, "w") as f:
        ...
        f.write(f"STEP_SIZE: 0.01 (baseline hard-coded)\n")
