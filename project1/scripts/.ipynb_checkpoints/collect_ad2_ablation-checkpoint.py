...
def infer_scale_and_K(name: str):
    # 支持 ad2_cells_1_0 / ad2_cells_1.5 / ad2_cells_K1 / ad2_cells_K2
    s = name.lower()
    m = re.search(r"(?:cells[_-]k)(\d+)", s)
    if m:
        K = int(m.group(1))
        scale = float(K)  # 粗略：K=ceil(scale)；只用来区分 K
        return scale, K
...
def collect(roots, outdir):
    ...
    # 画图：每个 N 一张 (ms/iter vs PT，K=1/2 两条线)
    for N, sub in df.groupby("N"):
        ...
        # 若 kernel 列有值，再画一张 kernel ms/iter
